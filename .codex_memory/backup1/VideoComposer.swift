...
final class VideoComposer {
    ...
    private static func copyrightWatermarkImage(
        ...
        let gap = 5 * scale
        let padding = 3 * scale
        let defaultText = settings.platform == .other ? "etc" : settings.platform.title
    ...
    private static func logoWatermarkImage(
        ...
        return UIGraphicsImageRenderer(
            ...
            if let logo = UIImage(named: "LogoMarkV2") {
                if settings.copyrightIconColorMode == .overlay {
                    let color = UIColor(
                        hexString: settings.copyrightIconColorHex
                    ) ?? HanClipTheme.primaryUIColor
    ...
    private static func drawWatermarkPlatformLogo(
        ...
        if let imageName = copyrightPlatformImageName(for: platform),
           let image = UIImage(named: imageName) {
            ...
            if settings.copyrightIconColorMode == .overlay {
                ...
                Self.drawImageAspectFit(image, in: drawRect, tintColor: color)
            } else {
        ...
        case .other:
            ...
            let logo = "etc" as NSString
            logo.draw(
                in: rect.insetBy(dx: 5 * scale, dy: 7 * scale),
                withAttributes: [
                    .font: UIFont.systemFont(
                        ofSize: 10 * scale,
                        weight: .heavy
